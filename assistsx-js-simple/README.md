...
### 1. 安装[AssistsX](https://www.pgyer.com/assistsx)

安装前请先在手机安装[AssistsX](https://www.pgyer.com/assistsx)

### 2. 插件包传至手机
下载本示例源码后（*前提`npm`环境配置好*）
1. 终端依次运行命令：`npm install`，`npm run build`
2. 编译完成后将`dist`文件夹压缩为`.zip`文件，然后将`.zip`文件传至手机。
3. 手机打开[AssistsX](https://www.pgyer.com/assistsx)，点击+号->本地添加->选择传至手机的`zip`压缩包
...
> 确保手机与电脑处于同一个局域网
### 1. 安装[AssistsX](https://www.pgyer.com/assistsx)
加载运行前请先在手机安装[AssistsX](https://www.pgyer.com/assistsx)

### 2. 启动项目
下载本示例源码后，终端依次运行命令：`npm install`，`npm run dev`

### 3. 加载插件
手机打开[AssistsX](https://www.pgyer.com/assistsx)，点击+号->扫描局域网->点击扫描到插件+号
